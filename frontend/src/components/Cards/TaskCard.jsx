...
const TaskCard = ({
  ...
  const completedTodos = todoChecklist?.filter((t) => t.completed).length || 0;
            ...
            {todoChecklist?.map((todo, index) => (
              <li key={index} className="flex items-center gap-2">
                <input
                  ...
                  disabled={status === "Completed"} // ✅ Lock when completed
                  onChange={(e) =>
                    onToggleTodo && onToggleTodo(index, e.target.checked)
                  }
